...
ローカルメールボックスは同一マシンおよび信頼された LAN 内の調整を解決します。実際のメールインフラによる組織間配信が必要になったら、プロトコルセマンティクスを [AAMP](https://github.com/larksuite/aamp)（Agent Asynchronous Messaging Protocol）へ昇格させてください —— agent-mailbox のメッセージライフサイクルは、それにクリーンにマッピングできるよう設計されています。

## 類似プロジェクトとの比較

| | agent-mailbox | [cc2cc](https://github.com/non4me/cc2cc) | [agent-talk](https://github.com/xhluca/agent-talk) | メールベースのキット |
|---|---|---|---|---|
| 外部サービス | **なし** | Claude Code channels | retalk relay | SMTP / IMAP / クラウド |
| オフライン動作 | **はい** | はい | relay が必要 | いいえ |
| 永続的アイデンティティ | **一度登録** | セッション バインド | 招待コード | プロバイダごと |
| 任意の MCP クライアント | **はい** | Claude Code のみ | 6 つの CLI、プラグインのみ | はい |
| 人間が読めるストア | **プレーン JSON** | JSON | relay 側 | メールボックスエクスポート |
...
- 改ざん耐性のあるレシート（ed25519 署名）はロードマップ上にあります —— パターンは [agenttransfer](https://github.com/shehryarsaroya/agenttransfer) を参照。
...
- **v1.0.0** —— AAMP ブリッジ：ローカルスレッドを [AAMP プロトコル](https://github.com/larksuite/aamp)経由の組織間メールへ昇格。
