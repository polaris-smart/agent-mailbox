...
- **v0.7.3** (actuelle) — hygiène sdist, deuxième passe : le balayage post-publication a repéré `scripts/wake-zc.sh` (un wrapper d'exploitation avec des chemins locaux codés en dur) dans les sdists 0.7.0–0.7.2 ; désormais exclu via hatchling `exclude` — le wheel ne l'a jamais embarqué. Détails (en anglais) : [README.md](README.md)
- **v0.4.0** (actuelle) — lot de fonctionnalités : style de signature webhook configurable (`AGENT_MAIL_SIGNATURE_STYLE` : github par défaut / generic / slack) ; protection contre l'auto-écho (les notifications où expéditeur == destinataire ne sont pas délivrées par défaut et sont auditées en `echo_suppressed` dans `sent.log` ; `notify_self_echo` / `AGENT_MAIL_NOTIFY_SELF_ECHO` restaure la livraison avec un préfixe `[echo] ` sur le sujet de la notification tandis que la lettre garde le sien) ; nouvelle commande de maintenance `cleanup --dry-run` (scanne les résidus de test, liste sans supprimer, `--yes` supprime après confirmation).
...
- **v0.7.0** — la mise à niveau de réveil : **sampling wake** (createMessage inverse via la connexion MCP de l'hôte — injection de politique, verrou par agent, timeout 60s, dédoublonnage, fail-open vers la boîte), **adaptateur local-command** (argv seul, contenu via variables d'env, killpg au timeout), `wake run --adapter`. 13 outils MCP.
...
- **v0.5.0+** — peut-être : intégrations kanban plus poussées (Kaneo comme référence/concurrent). En discussion.
