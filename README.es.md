...
- **v0.7.0** — la actualización de despertar: **sampling wake** (createMessage inverso por la conexión MCP del propio host — inyección de política, lock por agente, timeout 60s, dedup por mensaje, fail-open al buzón), **adaptador local-command** (solo argv, contenido por variables de entorno, killpg al agotar), `wake run --adapter`. 17 herramientas MCP.
