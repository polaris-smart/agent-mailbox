...
> 🆕 **v0.7.x — the wake-up upgrade**: **sampling wake** — the server pings the recipient's host over its own MCP connection (`sampling/createMessage`) the moment a letter lands, with per-agent wake-policy injection (identity template, forbidden hard constraints, per-agent execution lock — one clone in flight at a time) — plus the **local-command wake adapter** for CLI-only agents (codex et al., argv-only + env-injected + killpg timeout) and a `wake run --adapter` override for shared `wake.json` / per-agent plists. Sampling is an accelerator, never a delivery guarantee: MCP deprecated it on 2026-07-28 (SEP-2577), so v0.7.2 adds a per-agent kill switch (`"sampling": {"enabled": false}` in `wake.json`) while the fallback chain (wake-daemon / webhook / next check) keeps the mail-arrival iron law. 13 MCP tools. Previous: [Wake daemon](#wake-daemon-信必达-mail-arrived--agent-woken)
...
- **v0.7.3** (current) — sdist hygiene, round two: the post-release artifact sweep caught `scripts/wake-zc.sh` (a machine-specific ops wrapper with hardcoded local paths) shipping in the 0.7.0–0.7.2 sdists; now excluded via hatchling `exclude` — the wheel never carried it, the repo copy stays (launchd wiring untouched).
