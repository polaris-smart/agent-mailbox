...
> 🆕 **v0.7.x — Atualização de despertar**: **sampling wake** — assim que uma carta chega, o servidor chama o host do destinatário pela própria conexão MCP dele (`sampling/createMessage`), com injeção de política de despertar por agente (modelo de identidade, restrições proibidas, trava de execução por agente), além do **adaptador local-command** para agentes só-CLI (codex) e a sobrescrita `wake run --adapter`. O sampling é um acelerador, nunca uma garantia de entrega: o MCP o descontinuou em 2026-07-28 (SEP-2577), então a v0.7.2 adiciona um interruptor por agente (`"sampling": {"enabled": false}` no `wake.json`) e a cadeia de fallback (wake-daemon / webhook / próxima checagem) mantém a lei de ferro de entrega. 13 ferramentas MCP. Detalhes (em inglês): [README.md](README.md)
...
- **v0.7.3** (atual) — higiene de sdist, segunda rodada: a varredura pós-publicação pegou `scripts/wake-zc.sh` (um wrapper operacional com caminhos locais fixos) nos sdists 0.7.0–0.7.2; agora excluído via hatchling `exclude` — o wheel nunca o carregou. Detalhes (em inglês): [README.md](README.md)
- **v0.4.0** — lote de funcionalidades: estilo de assinatura de webhook configurável (`AGENT_MAIL_SIGNATURE_STYLE`: github padrão / generic / slack); proteções contra auto-eco (notificações onde remetente == destinatário não são entregues por padrão e são auditadas como `echo_suppressed` no `sent.log`; `notify_self_echo` / `AGENT_MAIL_NOTIFY_SELF_ECHO` restaura a entrega com prefixo `[echo] ` no assunto da notificação enquanto a carta mantém o seu); novo comando de manutenção `cleanup --dry-run` (escaneia resíduos de teste, lista sem excluir, `--yes` exclui após confirmação).
- **v0.3.1** — lote de correções: assuntos de resposta não acumulam mais `Re: Re:` (primeira resposta, re-respostas e prefixos com maiúsculas/minúsculas mistas normalizam para um único `Re:`); tokens do quadro web usam comparação em tempo constante (`hmac.compare_digest`) e persistem entre reinicializações (`~/.agent-mail/web_token`, modo 0600, a env `AGENT_MAIL_WEB_TOKEN` sempre vence); `sent.log` rotaciona automaticamente uma geração acima de 10 MB (para `sent.log.1`).
- **v0.7.0** — a atualização de despertar: **sampling wake** (createMessage reverso pela conexão MCP do próprio host — injeção de política, lock por agente, timeout 60s, dedup por msg, fail-open para a caixa), **adaptador local-command** (só argv, conteúdo via variáveis de ambiente, killpg no timeout), `wake run --adapter`. 13 ferramentas MCP.
...
- **v0.5.0+** — talvez: integrações kanban mais profundas (Kaneo como referência/concorrente). Em discussão.
