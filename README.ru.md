...
> 🆕 **v0.7.x — Обновление пробуждения**: **sampling wake** — как только письмо приходит, сервер вызывает хост получателя через его собственное MCP-соединение (`sampling/createMessage`), с принудительной инъекцией политики пробуждения на агента (шаблон личности, запреты, блокировка выполнения на агента), плюс **адаптер local-command** для CLI-агентов (codex) и переопределение `wake run --adapter`. Sampling — лишь ускоритель, но не гарантия доставки: MCP объявил его устаревшим 2026-07-28 (SEP-2577), поэтому v0.7.2 добавляет переключатель на агента (`"sampling": {"enabled": false}` в `wake.json`), а цепочка fallback (wake-daemon / webhook / следующая проверка) держит железный закон доставки. 13 MCP-инструментов. Подробности (на английском): [README.md](README.md)
...
- **v0.7.3** (текущая) — гигиена sdist, второй проход: пост-релизная проверка артефактов нашла `scripts/wake-zc.sh` (операционная обёртка с захардкоженными локальными путями) в sdists 0.7.0–0.7.2; теперь исключён через hatchling `exclude` — wheel его никогда не содержал. Подробности (на английском): [README.md](README.md)
- **v0.4.0** (текущая) — пакет функций: настраиваемый стиль подписи webhook (`AGENT_MAIL_SIGNATURE_STYLE`: github по умолчанию / generic / slack); защита от авто-эха (уведомления, где отправитель == получатель, по умолчанию не доставляются и аудируются как `echo_suppressed` в `sent.log`; `notify_self_echo` / `AGENT_MAIL_NOTIFY_SELF_ECHO` восстанавливает доставку с префиксом `[echo] ` в теме уведомления, само письмо сохраняет свою тему); новая сервисная команда `cleanup --dry-run` (сканирует тестовые остатки, только перечисляет без удаления, `--yes` удаляет после подтверждения).
- **v0.3.1** — патч-батч: темы ответов больше не накапливают `Re: Re:` (первый ответ, повторные ответы и префиксы в любом регистре нормализуются к одному `Re:`); токены веб-доски используют сравнение за постоянное время (`hmac.compare_digest`) и сохраняются между перезапусками (`~/.agent-mail/web_token`, режим 0600, переменная окружения `AGENT_MAIL_WEB_TOKEN` всегда имеет приоритет); `sent.log` автоматически ротируется на одно поколение после 10 МБ (в `sent.log.1`).
- **v0.7.0** — обновление пробуждения: **sampling wake** (обратный createMessage через собственное MCP-соединение хоста — инъекция политики, блокировка на агента, таймаут 60с, дедупликация, fail-open в ящик), **адаптер local-command** (только argv, содержимое через переменные окружения, killpg по таймауту), `wake run --adapter`. 13 инструментов MCP.
- **v0.3.0** — доска задач + веб-канбан: `task_create` / `task_move` / `task_list` со строгой машиной состояний todo→doing→review→done; создание или перемещение карточки автоматически уведомляет ответственного, движение доски будит агентов без всякого опроса. `--web 8643` поднимает канбан-интерфейс без зависимостей, защищённый токеном, где человеческое перетаскивание идёт по тому же пути пробуждения. Сообщения + задачи + пробуждение + доска, по-прежнему ноль зависимостей.
- **v0.5.0+** — возможно: более глубокие интеграции с канбан-системами (Kaneo как референс/конкурент). На обсуждении.
