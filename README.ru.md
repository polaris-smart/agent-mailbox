...
> 🆕 **v0.6.2 — Укрепление безопасности**: опциональный `identity_binding` привязывает вызывающих MCP к id агентов через `AGENT_MAIL_TOKEN` — семантика зафиксирована: **не включено → разрешить (локальное доверие остаётся по умолчанию); неверный токен → отказ с `identity mismatch`; повреждённая конфигурация → громкий сбой при запуске**.. Полезная нагрузка webhook теперь несёт `unread_count` получателя, а `mailbox_wait` атомарно заявляет письма (`acked` + `claimed_by` за один проход под блокировкой) — два waiters никогда не потребляют одну и ту же партию. Модель и отчётность: [SECURITY.md](SECURITY.md). Подробности (на английском): [README.md](README.md)
