...
> 🆕 **v0.6.0 — Wake daemon + Threads + Jev-маршрутизация**: `agent-mailbox wake install` превращает «письмо доставлено» в «агент разбужен» через файловые наблюдатели launchd/systemd — встроены повтор, дедупликация и fail-open. Потоки (threads) первого класса (`mailbox_thread`, автоматический `thread_id`, обратное заполнение старых цепочек `Re:`, предупреждения о «призрачных» тредах) и опциональный Jev-роутер (по умолчанию выключен), оценивающий, стоит ли вас будить. Всего 13 инструментов MCP. ⚠️ Перезапустите сессию агента, чтобы новые инструменты появились. → Подробности (на английском): [README.md](README.md)
