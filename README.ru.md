...
- **v0.7.0** — обновление пробуждения: **sampling wake** (обратный createMessage через собственное MCP-соединение хоста — инъекция политики, блокировка на агента, таймаут 60с, дедупликация, fail-open в ящик), **адаптер local-command** (только argv, содержимое через переменные окружения, killpg по таймауту), `wake run --adapter`. 17 инструментов MCP.
